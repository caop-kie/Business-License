...
class Renderer(object):
    def __init__(self, corpus, fonts, bgs, cfg, width=256, height=32,
        ...
        self.entity_loc = {
            'social_credit_code1': [140, 380],    # 社会统一信用代码 [489, 435]
            'social_credit_code2': [236, 384],
            'name': [293, 535],   # 名称
            'type': [293, 586],   # 类型
            'legal_person': [293, 641],    # 合伙人
            'business_scopes': [293, 694],  # 经营范围
            'start_date': [1000, 586],  # 成立日期
            'operation_date': [1000, 641],
            'location': [1000, 693],
            'registration_capital': [1000, 530]
        }

    def generate_entity_loc_shift(self):
        d = {}
        for k, v in self.entity_loc.items():
            shift_x = random.randint(-10, 10)
            shift_y = random.randint(-10, 10)
            d[k] = [v[0] + shift_x, v[1] + shift_y]
        return d
    # [
    #     'company_name'， 'date'， ...
    # ]
    def gen_img_new(self, img_index, company_name=None, bg=None, entity_type=None, entity_dict=None, ocr=None):
        font_path = random.choice(self.fonts)
        if entity_type != 'social_credit_code':
            font_size = self.cfg.font_size.normal
        else:
            font_size = self.cfg.font_size.min
        font = ImageFont.truetype(font_path, font_size)
        new_entity_loc = self.generate_entity_loc_shift()
        if entity_type != 'social_credit_code':
            loc = new_entity_loc[entity_type]
        else:
            if random.randint(0, 1) == 0:
                loc = new_entity_loc['social_credit_code1']
            else:
                loc = new_entity_loc['social_credit_code2']

        company_name = self.corpus.get_sample('name', None) if not company_name else company_name
        # for key in entity_type:
        key = entity_type
        if key != 'name':
            word = self.corpus.get_sample(key, company_name)
        else:
            word = company_name
        if key == 'location' or key == 'business_scopes':
            s = 0
            line_max_length = 23
            # if key == 'location':
            #     prefix = word.split(' ')[0]
            #     if random.choice([0, 1]) == 0:
            #         prefix = prefix + word.split(' ')[1]
            #     word_img, text_box_pnts, word_color = self.draw_text_on_bg_new(prefix + '市场监督管理局', font, bg, new_entity_loc['register_office'])
            #     bg = word_img
            #     ocr.append(','.join(text_box_pnts) + ',' + prefix + '市场监督管理局'+',register_office')
            #     entity_dict['register_office'] = prefix + '市场监督管理局'
            word = word.replace(' ', '')
            # word = word.replace('，', '， ')
            while s < len(word):
                e = min(s + line_max_length, len(word))
                word_img, text_box_pnts, word_color = self.draw_text_on_bg_new(word[s:e], font, bg, loc)
                ocr_line = word[s:e] + ','
                if key == 'location':
                    ocr_line += 'location'
                elif key == 'business_scopes':
                    ocr_line += 'business_scopes'
                ocr.append(','.join(text_box_pnts) + ',' + ocr_line)
                loc = [loc[0], loc[1] + font_size + 2]  # +2 留点空间
                s = s + line_max_length
                bg = word_img
            entity_dict[key] = word
        elif key == 'operation_date':
            date1, date2 = word.split('，')[:]
            word_img, text_box_pnts, word_color = self.draw_text_on_bg_new(date1, font, bg, new_entity_loc['start_date'])
            ocr.append(','.join(text_box_pnts) + ',' + word + ',start_date')
            word_img, text_box_pnts, word_color = self.draw_text_on_bg_new(date1 + ' 至 ' + date2, font, word_img, loc)
            ocr.append(','.join(text_box_pnts) + ',' + date1 + '至' + date2 + ',operation_date')

            # fake = Faker()
            # date1_date = datetime.strptime(date1, '%Y年%m月%d日')
            # license_date = fake.date_time_between(start_date=date1_date, end_date='+5y')
            # word_img, text_box_pnts, _ = self.draw_text_on_bg_new(str(license_date.year), font, word_img, new_entity_loc['license_date_year'])
            # ocr.append(','.join(text_box_pnts) + ',' + str(license_date.year) + ',other')
            # word_img, text_box_pnts, _ = self.draw_text_on_bg_new(str(license_date.month), font, word_img, new_entity_loc['license_date_month'])
            # ocr.append(','.join(text_box_pnts) + ',' + str(license_date.month) + ',other')
            # word_img, text_box_pnts, _ = self.draw_text_on_bg_new(str(license_date.day), font, word_img, new_entity_loc['license_date_day'])
            # ocr.append(','.join(text_box_pnts) + ',' + str(license_date.day) + ',other')
        else:
            word_img, text_box_pnts, word_color = self.draw_text_on_bg_new(word, font, bg, loc)
            ocr.append(','.join(text_box_pnts) + ',' + word + ',' + entity_type)
            entity_dict[key] = word

        return company_name, word_img, word, entity_dict, ocr
    ...
    def draw_text_on_bg_new(self, word, font, bg, loc):
        ...
        word_color = self.get_black_word_color()
        self.draw_text_wrapper(draw, word, loc[0] - offset[0], loc[1] - offset[1], font, word_color)
        ...
        text_box_pnts = [
            [loc[0], loc[1]],
            [loc[0] + word_width, loc[1]],
            [loc[0] + word_width, loc[1] + word_height],
            [loc[0], loc[1] + word_height]
        ]
        np_img = np.clip(np_img, 0., 255.)
        string_text_points = [str(item) for sublist in text_box_pnts for item in sublist]
        return np_img, string_text_points, word_color
    ...
    def gen_bg_from_image(self, width, height):
        ...
        # 423,550
        # return out
        return bg[550:550+height, 423:423+width]
